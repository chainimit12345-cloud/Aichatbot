document.addEventListener("DOMContentLoaded", () => {
  ...
  // 🔴 เพิ่มลูกเล่นการแปลงร่างปุ่ม 3 ขีด เป็นรูปกากบาท (X)
  function toggleMenu() {
    if (window.innerWidth < 1024) {
      // เลื่อนเปิด/ปิด Sidebar
      sidebar.classList.toggle("-translate-x-full");
      overlay.classList.toggle("hidden");

      // เปลี่ยนขีดเป็นกากบาท
      if (hamburgerBtn) {
        ...
        setTimeout(
          () =>
            ...
            <div class="mb-1">
                ...
                <a href="forms/แบบคําขอรับเงินสงเคราะห์และรับรองผู้รับผิดชอบในการจัดการศพผู้สูงอายุตามประเพณี.pdf" download target="_blank" class="inline-flex items-center gap-2 bg-red-50 text-red-600 border border-red-200 px-3.5 py-2 rounded-xl text-[12px] font-bold hover:bg-red-50 hover:text-white hover:bg-red-500 transition shadow-sm">
  ...
  if (newChatBtn) {
    newChatBtn.addEventListener("click", () => {
      ...
      if (
        ...
        toggleMenu(); // สั่งให้ตอนกด New Chat เมนูก็จะหมุน X กลับเป็น 3 ขีดและปิดไปเองอัตโนมัติ
      }
    });
  }
