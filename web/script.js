document.addEventListener("DOMContentLoaded", () => {
    ...
    function renderSidebarData() {
        ...
        if(document.getElementById('forms-container')) document.getElementById('forms-container').innerHTML = formsHTML;
            ...
            return new Promise(resolve => {
                setTimeout(() => {
                    if (typeof LAW_CHILD_KNOWLEDGE !== 'undefined') {
                        let formattedText = LAW_CHILD_KNOWLEDGE
                            .replace(/\*\*(.*?)\*\*/g, '<br><strong class="text-primary text-[13px] xs:text-[14px]">$1</strong>')
                            .replace(/\n/g, '<br>');
                        ...
                        return resolve('ขออภัยค่ะ ไม่พบฐานข้อมูลกฎหมายในระบบ โปรดตรวจสอบไฟล์ knowledge.js ค่ะ');
                    }
                }, 500);
            });
        }
            ...
            if (typeof LAW_CHILD_KNOWLEDGE !== 'undefined' && text.length > 1) {
                ...
                if (foundResults.length > 0) {
                    ...
                    limitResults.forEach(line => {
                        ...
                        let highlightedLine = line.replace(highlightRegex, `<span class="bg-yellow-200 text-gray-900 px-1 rounded font-semibold">${text}</span>`);
